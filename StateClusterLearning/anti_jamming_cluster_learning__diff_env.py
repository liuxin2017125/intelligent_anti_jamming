...
if __name__ == '__main__':
    ...
    number_of_states = 20
    env = Environment(10)
    [node0, node1, jammer, shape] = createScenario(env, scenario_index)
    ...
    simu_times = 40000
    watchPoints = [100, round(simu_times * 0.4), round(simu_times * 0.8), round(simu_times * 0.95)]
    waterfallList = []
    ...
    for t in range(0, simu_times):
        ...
        if t in watchPoints:
            ...
            pass
        if t == watchPoints[2] - 100:
            jammer.setOffset(6)  # change the jammer parameter

    filename = '.\\records\\cluster_learning_diff_env_s%d_n%d.mat' % (scenario_index, number_of_states)
